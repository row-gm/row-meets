...
function render() {{
  ...
  if (!showM) {{
    ...
    /* First four columns match the events table below, so a reader moving
       between them finds the same thing in the same place. Each table's own
       columns follow. */
    var head = ['Date','Name','Location','Confirm By','Pool','Type','Eligibility'];
    ...
    list.forEach(function (m) {{
      ...
      h += '<tr>' +
        ...
        '<td data-label="Name"><span class="name">' + name + '</span>' + extra + '</td>' +
        '<td data-label="Location">' + esc(m.city) +
        ...
        '<td data-label="Eligibility">' + eligFor(m, code).map(function (e) {{
  ...
  if (!evs.length) {{
    ...
    var eh = '<table><thead><tr>' +
      ['Date','Name','Location','Confirm By','Time','Type'].map(th).join('') +
      '</tr></thead><tbody>';
    evs.forEach(function (e) {{
      ...
      var nm = eurl
        ...
        : esc(e.name);
      /* The type tag moves to its own column, matching the meets table, so it
         is no longer buried under the name. */
      var extra = '';
      ...
      eh += '<tr>' +
        ...
        '<td data-label="Name"><span class="name">' + nm + '</span>' + extra + '</td>' +
        '<td data-label="Location">' + (e.location ? esc(e.location) : '&mdash;') + '</td>' +
        '<td data-label="Confirm by" class="date">' + confirmLabel(e) + '</td>' +
        '<td data-label="Time" class="date">' + time + '</td>' +
        '<td data-label="Type">' +
          tag(e.type, data.eventTypes[e.type] || '{INK_SOFT}') + '</td>' +
        '</tr>';
    }});
